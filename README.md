...
Запуск:
  python main.py [--name|-n]=<file.lst>|<file.xsl>|<file.zip> [[--inn|-i]=<inn>] 
  [[--config|-c]=<config.ini>] [[--union|-u]=<path>']  [[--hash|-x]=no|yes']
...
--hash=использовать(yes) или нет (no) hash-функцию, где она задана (по-умолчанию yes) 
...
value=строка                        ;значение, присваиваемое параметру при срабатывании шаблона
...
[col_x] ; x-порядковый номер колонки в диапазоне [0,] и/или [20,]
