...
def get_config_files():
    try:
        ...
        files = sorted(
            files,
            key=lambda x: (x[10:13], x[14:17] if x[16:17] != "." else x[14:16] + "я"),
        )
...
def get_files(file_name: str, inn: str, file_conf: str) -> list:
    ...
    if zip_files:
        for file_name in zip_files:
            ...
            file_name["config"] = [file_conf] if file_conf else file_name["config"]
...
def get_file_config(list_files: list) -> str:
    ...
    for item in list_files:
        ...
        if item["config"]:
            ...
            ls_new.append(__config_find(data_file, config_files, i, len(list_files)))
        i += 1
    return ls_new
    ...
    for conf_file in config_files:
        print_message(
            "          Поиск конфигураций: {}%   \r".format(
                round((j * len(config_files) + i) / (m * len(config_files)) * 100, 0)
            ),
            end="",
            flush=True,
        )
...
def __config_process(data_file: dict, file_config):
    rep = Report_001_00(
        file_name=data_file["name"], config_file=str(file_config), inn=data_file["inn"]
    )
