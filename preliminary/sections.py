...
def write_section_org_ppa_guid(
    file, lines: dict, sec_type: str, sec_number: int, sec_title: str, sec_name: str
):
    ...
    if True or lines["dic"].get("rr1_puv"):
        ...
        if lines["dic"].get(f"{sec_name}_{sec_type}"):
            ...
            if is_ident:
                ...
                file.write(f'func={get_func_name(l[0]["name"].split(";")[0])},hash\n')
        if lines["dic"].get(f"{sec_name}_{sec_type}"):
            for i, line in enumerate(lines["dic"][f"{sec_name}_{sec_type}"][1:]):
                ...
                if is_ident:
                    ...
                    file.write(f'func={get_ident(line["name"])},hash\n')
        else:
            file.write("\n")
            __write_sub_fields(
                file=file,
...
def write_section_internal_id(
    ...
    sec_name: str,
    sec_suffix: str = "",
    sec_is_service: bool = True,
    sec_is_hash: bool = True,
):
    __write_section_head(file, sec_type, sec_number, sec_title, sec_name)
    if sec_is_service:
        __write_section_service(
            file,
            lines,
            sec_type,
            sec_number,
...
def write_section_address(
    ...
    if lines["dic"].get(f"{sec_name}"):
        file.write("pattern=.+\n")
        file.write(f'col_config={lines["dic"][f"{sec_name}"][0]["col"]}\n')
        file.write("row_data=0\n")
...
def write_section_account_internal_id(
    file, lines: dict, sec_type: str, sec_number: int, sec_title: str, sec_name: str
):
    __write_section_head(file, sec_type, sec_number, sec_title, sec_name)
    file.write("pattern=@0\n")
    ...
    file.write("row_data=0\n")
    if lines["dic"].get(f"{sec_name}"):
        file.write(f'offset_col_config={lines["dic"][f"{sec_name}"][0]["col"]}\n')
        file.write("offset_pattern=.+\n")
    if lines["dic"].get(f"{sec_name}") and lines["dic"][f"{sec_name}"][0]["func"]:
        file.write(f'func={lines["dic"][f"{sec_name}"][0]["func"][0]}\n')
    else:
...
def write_section_service_internal_id(
    file,
    lines: dict,
    sec_type: str,
    sec_number: int,
    ...
    __write_section_head(file, sec_type, sec_number, sec_title, sec_name)
    __write_section_service(
        file, lines, sec_type, sec_number, sec_title, "internal_id", "ПУ", False
    )
    return


def write_section_rr(
    file,
    lines: dict,
    sec_type: str,
    sec_number: int,
    ...
    if lines["dic"].get(f"{sec_name}{sec_prefix}"):
        if lines["dic"].get("service"):
            ...
            for i, line in enumerate(lines["dic"][f"{sec_name}{sec_prefix}"][1:]):
                file.write(f"[{sec_type}_{sec_number}_{i}]\n")
                file.write(f"; {sec_title}\n")
                file.write(f'offset_col_config={line["col"]}\n')
    file.write("\n")
    return
...
def write_section(
    ...
    l: list = get_lines(lines)
    is_sub_fld = (
        lines["dic"].get("service")
        and sec_name in required_fields
        and len(lines["dic"][f"{sec_name}{sec_prefix}"]) == 1
    )
    if sec_prefix == "":
        sec_prefix = f"_{sec_type}"
    if lines["dic"].get(f"{sec_name}{sec_prefix}") is None:
    ...
    if lines["dic"].get(f"{sec_name}{sec_prefix}"):
        ...
        if lines["dic"].get(f"{sec_name}{sec_prefix}")[0]["pattern"]:
            ...
            if is_sub_fld:
                ...
                if not sec_func:
                    file.write(
                        f'func=column_value({lines["dic"][f"{sec_name}{sec_prefix}"][0]["col"]})\n'
                    )
            else:
                file.write("offset_pattern=.+\n")
...
def __write_section_head(
    file, sec_type: str, sec_number: int, sec_title: str, sec_name: str
):
    file.write(f"[{sec_type}_{sec_number}]\n")
    file.write(f"; {sec_title}\n")
    file.write(f"name={sec_name}\n")
...
def __write_sub_fields(
    ...
    for i, line in enumerate(l[1:]):
        ...
        if is_ident:
            ...
            if is_func:
                file.write(f'func={get_func_name(line["name"].split(";")[0])},hash\n')
